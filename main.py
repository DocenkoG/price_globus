...
def convert_excel2csv(cfg):
    ...
    subgrp = ''
    grp = ''
    grpind = in_cols_j['группа_']                                        # xls
#   for i in range(1, sheet.max_row +1) :                                # xlsx
    for i in range(9, sheet.nrows) :                                     # xls
        i_last = i
        try:
            xfx = sheet.cell_xf_index(i, grpind)                         # xls
...
#           ccc1 = sheet.cell(row=i, column=in_cols_j['цена1']).value
            ...
            if (font.bold == 1):                                      # группа
                grp = impValues['группа_']
                continue
            elif (impValues['код_'] == '' or                             # лишняя строка
#               impValues['код_'] == 'SAP' or
                sheet.cell(i, in_cols_j['цена1']).value is None):        # xls
#               impValues['цена1'] == '0'):                              # xlsx
                continue
            impValues['группа_'] = grp
            impValues['наименование'] = impValues['наименование'].encode('cp1251', errors='replace').decode('cp1251')
            ...
            for outColName in out_template.keys() :
                ...
                if (outColName == 'продажа') and ('*' in shablon) :
                    p = shablon.find("*")
